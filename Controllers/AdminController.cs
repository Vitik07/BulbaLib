    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> ProcessNovelRequest(int requestId, bool approve, string moderationComment)
        {
                ...
                try
                {
                    switch (request.RequestType)
                    {
                        case ModerationRequestType.NovelCreate:
                            ...
                            {
                                Title = novelCreateData.Title, Description = novelCreateData.Description, AuthorId = novelCreateData.AuthorId,
                                Genres = novelCreateData.Genres, Tags = novelCreateData.Tags, Type = novelCreateData.Type,
                                Format = novelCreateData.Format, ReleaseYear = novelCreateData.ReleaseYear, AlternativeTitles = novelCreateData.AlternativeTitles,
                                RelatedNovelIds = novelCreateData.RelatedNovelIds, Date = novelCreateData.Date, Status = novelCreateData.Status,
            ...
            else // Reject
            {
                ...
                } else if (!string.IsNullOrEmpty(request.RequestData)) { // Fallback for delete if title was in RequestData
                ...
                try
                {
                    switch (request.RequestType)
                    {
                        case ModerationRequestType.ChapterCreate:
                            ...
                            if (request.NovelId.HasValue) { // Auto add TranslatorId to Novel
